...
use Procuracion\Entity\Unidadadministrativa;
...
class UnidadadministrativaService {
    ...
    public function listAll(EntityManager $em) {
        ...
        $listado = $em->getRepository('\Procuracion\Entity\Unidadadministrativa')->findBy(array(), array('nombre' => 'ASC'));
        return $listado;
    }

    public function listOne(EntityManager $em, $id) {
        $usuario = $em->getRepository('\Procuracion\Entity\Unidadadministrativa')->find($id);
        //return new JsonModel($enCola);
        return $usuario;
    }

    public function findByNombre(EntityManager $em, array $usr) {
        $data = $em->getRepository('Procuracion\Entity\Unidadadministrativa')->findByNombre($usr['nombre']);
        return $data;
    }
